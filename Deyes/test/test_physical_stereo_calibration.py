...
def test_board_shape_is_required_for_capture_and_validation() -> None:
    assert validate_capture_arguments(
        square_size_m=0.020, requested_samples=50, width=640, height=360, board_inner_corners=(8, 7)
    ) == ()
    gate = validation_gate(
        ...
        scale_confirmed=True, coverage_complete=True, board_inner_corners=(8, 3), square_size_m=0.020,
    )
    assert not gate.validated
    ...
    metric_gate = validation_gate(
        ...
        source="physical_checkerboard", left_right_confirmed=True, baseline_sign_confirmed=True,
        scale_confirmed=True, coverage_complete=True, board_inner_corners=(8, 7), square_size_m=0.020,
    )
...
def test_observation_loader_rejects_sample_board_mixed_with_session(tmp_path: Path) -> None:
    manifest = {
        "samples": [{
            "board_inner_corners": [9, 6], "pair_skew_ms": 0.0, "left": "left.png", "right": "right.png",
        }],
    }
    try:
        load_observations(tmp_path, manifest, (8, 7))
...
def test_compute_rejects_legacy_9x6_manifest_for_official_8x7_session(tmp_path: Path) -> None:
    ...
    args = argparse.Namespace(
        session_dir=str(tmp_path), robot_id="robot", camera_pair_id="pair", square_size_m=0.020,
        board_cols=8, board_rows=7, confirm_left_right=True, confirm_baseline_sign=True, confirm_scale=True,
    )
    try:

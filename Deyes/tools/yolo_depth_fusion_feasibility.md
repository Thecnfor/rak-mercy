...
> 历史记录，非当前双目点云部署目标。本文所有 `192.168.137.17` 结论只对应旧机；当前机器人地址仅由 shell 环境变量 `ROBOT_IP=192.168.166.121` 提供，且第一版不部署 YOLO。
...
- 缺点：
  ...
  - 历史 `192.168.137.17` 上的 `cv2 4.2.0` 已被实测证明无法正确导入 `yolov5s.onnx`
- 适用：
